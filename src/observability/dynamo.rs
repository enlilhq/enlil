...
impl DynamoTraceStore {
    ...
    pub async fn record(&self, trace: &Trace) {
        ...
        if let Err(e) = self.client.put_item().table_name(&self.table).set_item(Some(item)).send().await {
    ...
    pub async fn enrich_usage(&self, trace_id: &str, total_tokens: u32, cost_microdollars: u64, response_hash: Option<String>) {
        ...
        values.insert(":tt".to_string(), AttributeValue::N(total_tokens.to_string()));
        values.insert(":cm".to_string(), AttributeValue::N(cost_microdollars.to_string()));
        ...
        key.insert("trace_id".to_string(), AttributeValue::S(trace_id.to_string()));
    ...
    pub async fn get_scoped(&self, trace_id: &str, caller_tenant: &str, super_admin: bool) -> Option<Trace> {
        ...
        let result = self.client.get_item().table_name(&self.table).set_key(Some(key)).send().await;
    ...
    pub async fn list(&self, caller_tenant: &str, super_admin: bool, limit: usize) -> Vec<TraceSummary> {
        if super_admin {
            let result = self.client.scan().table_name(&self.table).limit(limit as i32).send().await;
            let mut items: Vec<TraceSummary> = match result {
                Ok(out) => out.items.unwrap_or_default().iter().filter_map(item_to_trace).map(|t| TraceSummary::from(&t)).collect(),
                Err(e) => {
...
fn trace_to_item(t: &Trace) -> HashMap<String, AttributeValue> {
    ...
    item.insert("trace_id".to_string(), AttributeValue::S(t.trace_id.clone()));
    item.insert("timestamp".to_string(), AttributeValue::N(t.timestamp.to_string()));
    item.insert("tenant_id".to_string(), AttributeValue::S(t.tenant_id.clone()));
    item.insert("session_id".to_string(), AttributeValue::S(t.session_id.clone()));
    item.insert("agent_id".to_string(), AttributeValue::S(t.agent_id.clone()));
    item.insert("method".to_string(), AttributeValue::S(t.method.clone()));
    item.insert("path".to_string(), AttributeValue::S(t.path.clone()));
    item.insert("protocol".to_string(), AttributeValue::S(t.protocol.clone()));
    item.insert("upstream".to_string(), AttributeValue::S(t.upstream.clone()));
    item.insert(
        "steps".to_string(),
    ...
    item.insert("status".to_string(), AttributeValue::N(t.status.to_string()));
    item.insert("blocked".to_string(), AttributeValue::Bool(t.blocked));
    if let Some(reason) = &t.block_reason {
        item.insert("block_reason".to_string(), AttributeValue::S(reason.clone()));
    }
    item.insert("latency_us".to_string(), AttributeValue::N(t.latency_us.to_string()));
    item.insert("total_tokens".to_string(), AttributeValue::N(t.total_tokens.to_string()));
    item.insert("cost_microdollars".to_string(), AttributeValue::N(t.cost_microdollars.to_string()));
    if let Some(hash) = &t.response_hash {
        item.insert("response_hash".to_string(), AttributeValue::S(hash.clone()));
    }
    item.insert("replay_uri".to_string(), AttributeValue::S(t.replay_uri.clone()));
    item.insert("replay_body".to_string(), AttributeValue::B(t.replay_body.clone().into()));
    item.insert("replay_method".to_string(), AttributeValue::S(t.replay_method.clone()));
    item
}
...
fn item_to_trace(item: &HashMap<String, AttributeValue>) -> Option<Trace> {
    ...
    Some(Trace {
        ...
        agent_id: item.get("agent_id").and_then(|v| v.as_s().ok()).cloned().unwrap_or_default(),
        method: item.get("method").and_then(|v| v.as_s().ok()).cloned().unwrap_or_default(),
        path: item.get("path").and_then(|v| v.as_s().ok()).cloned().unwrap_or_default(),
        protocol: item.get("protocol").and_then(|v| v.as_s().ok()).cloned().unwrap_or_default(),
        upstream: item.get("upstream").and_then(|v| v.as_s().ok()).cloned().unwrap_or_default(),
        steps,
        cache: item.get("cache").and_then(|v| v.as_s().ok()).cloned().unwrap_or_default(),
        status: item.get("status").and_then(|v| v.as_n().ok()).and_then(|s| s.parse().ok()).unwrap_or(0),
        blocked: item.get("blocked").and_then(|v| v.as_bool().ok()).copied().unwrap_or(false),
        block_reason: item.get("block_reason").and_then(|v| v.as_s().ok()).cloned(),
        latency_us: item.get("latency_us").and_then(|v| v.as_n().ok()).and_then(|s| s.parse().ok()).unwrap_or(0),
        total_tokens: item.get("total_tokens").and_then(|v| v.as_n().ok()).and_then(|s| s.parse().ok()).unwrap_or(0),
        cost_microdollars: item.get("cost_microdollars").and_then(|v| v.as_n().ok()).and_then(|s| s.parse().ok()).unwrap_or(0),
        response_hash: item.get("response_hash").and_then(|v| v.as_s().ok()).cloned(),
        replay_uri: item.get("replay_uri").and_then(|v| v.as_s().ok()).cloned().unwrap_or_default(),
        replay_body: item.get("replay_body").and_then(|v| v.as_b().ok()).map(|b| b.clone().into_inner()).unwrap_or_default(),
        replay_method: item.get("replay_method").and_then(|v| v.as_s().ok()).cloned().unwrap_or_else(|| "POST".to_string()),
    })
}
